...
lincs learn classification-model problem.yml learning-set.csv --model-type ucncs --ucncs.approach sat-by-coalitions >learned-model-without-chrones.yml

if test -f run-result.json; then false; fi
if test -f lincs.*.chrones.csv; then false; fi

chrones run -- lincs learn classification-model problem.yml learning-set.csv --model-type ucncs --ucncs.approach sat-by-coalitions >learned-model-with-chrones.yml
