...
LinearProgram make_internal_linear_program(const float epsilon, const ModelsView& models, uint model_index) {
  LinearProgram lp;
...
std::shared_ptr<glp::LinearProgram> make_verbose_linear_program(
  ...
  auto models = models_.get_view();
  LinearProgram lp = make_internal_linear_program(epsilon, models, model_index);
