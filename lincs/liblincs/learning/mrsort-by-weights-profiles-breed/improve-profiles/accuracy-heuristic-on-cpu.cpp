...
void ImproveProfilesWithAccuracyHeuristicOnCpu::improve_model_profile(
  ...
  unsigned best_destination_rank = learning_data.low_profile_ranks[model_index][profile_index][criterion_index];
  ...
  if (std::uniform_real_distribution<float>(0, 1)(learning_data.urbgs[model_index]) <= best_desirability) {
    learning_data.low_profile_ranks[model_index][profile_index][criterion_index] = best_destination_rank;
  }
}
...
void ImproveProfilesWithAccuracyHeuristicOnCpu::update_move_desirability(
  ...
  const unsigned current_rank = learning_data.low_profile_ranks[model_index][profile_index][criterion_index];
